...
$getData = "SELECT * FROM speech_for_user WHERE status = 1 AND id = '".$_REQUEST['spch_id']."' ORDER BY id DESC ";
...
if (isset($_POST['save_btn'])) {
    ...
    $insertQry = "INSERT INTO speech_to_speech_by_user SET speech_id = '".$_REQUEST['spch_id']."', speech_by_user = '".$user_txt."', user_id = '".$user_id."', matching_characters = '".$similar_char."', matching_accuracy_result = '".$percent."' ";
    ...
    if ($insertExc) {
        ...
        echo "<script>window.location.href='speech_to_speech.php';</script>";
    }
}
                ...
                <div class="col-xl-12">
                    ...
                    if(isset($_SESSION['ocaWebUser']) && !empty($_SESSION['ocaWebUser']))
                    {
                        ...
                        foreach ($speechArr as $key => $value) {
                            $getUserTextData = "SELECT * FROM speech_to_speech_by_user WHERE status = 1 AND user_id = '".$_SESSION['ocaWebUser']['id']."' AND speech_id = '".$value['id']."' ORDER BY id DESC";
                        ...
                        <?php }else{?> 
                            <h1 style="text-align: center;"><?php echo $value['speech_title']; ?></h1><br><br>
                            <form action="" method="post" autocomplete="off">
                                <input type="hidden" name="texttospeech" value="<?php echo $value['speech_content']; ?>">
                                <input type="hidden" name="hidden_txt_from_user" id="note-textarea" value="">
                                <h3>Listen carefully this audio first and then you have to repeat it.</h3>
                                
                                <input type = "submit" class = "btn btn-warning" name="process" value="Play Audio">

                                <input type="button" id="start-record-btn" class="btn btn-success" title="Start Recording" value="Repeat It">

                                <button type="submit" name="save_btn" class="btn btn-info" title="Submit Speech">Submit Speech</button> <br><br>
                                <p id="recording-instructions">Press the <strong>Repeat It</strong> button and allow access.</p><br><br>
                            </form>
                        <?php }
                        $cnt++; } 
                    }else{ ?>
                    ...
                    <div style="height: 120px">
                        <a href="speech_to_speech.php" class="btn btn-dark" style="width: 150px;height: auto;"><i class="fa fa-arrow-left"></i>&nbsp;Back</a>
